...
class TestCheckout:
    ...
    @pytest.mark.parametrize("basket,value", [("VVVVV", 220), ("UUUU", 120), ("UUU", 120), ("RRRQ", 150), ("RRR", 150), ("RRRQQQ", 210), ("QQQPPPPP", 280), ("MNNN", 120), ("NNN", 120), ("KK", 120)])
    def test_new_products(self, basket, value):
        assert checkout_solution.checkout(basket) == value

    @pytest.mark.parametrize("basket,value", [("XYZ", 45), ("XYZXYZ", 90), ("XYZST", 82), ("YXYZST", 90), ("YXXYZST", 107)])
    def test_group_discount(self, basket, value):
        assert checkout_solution.checkout(basket) == value

    @pytest.mark.parametrize("basket,value",
                             [("K", 70), ("ABCDEFGHIJKLMNOPQRSTUVW", 795)])
    def test_failures(self, basket, value):
        assert checkout_solution.checkout(basket) == value